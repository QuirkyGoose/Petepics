...
/* ── Main Page ────────────────────────────────────────────── */
export default function GalleryView({ initialRoom = "all" }: { initialRoom?: string }) {
  ...
  /* Browser Back closes the lightbox (synced via pushState in openLightbox) */
  useEffect(() => {
    function onPopState() {
      // User hit Back — close lightbox without calling history.back() again
  ...
  /* NOTE: Hash-based deep linking removed — now handled by the /gallery/[room] route */
  ...
  /* Room change handler — push new route + reset pagination + scroll to top. */
  const handleRoomChange = useCallback((room: string) => {
    setCurrentRoom(room);
    setSearchQuery("");
    setMobileMenuOpen(false);
    setDisplayCount(PAGE_SIZE); // reset pagination on room switch
    window.scrollTo({ top: 56, behavior: "smooth" });
    setRoomTransitionKey((k) => k + 1);
    // Update URL via Next.js router (multi-route architecture)
    router.push(`/gallery/${room}`);
  }, [router]);
  ...
  const openLightbox = useCallback(
    (items: GalleryWork[], index: number) => {
      ...
      if (work) {
        addRecent(work.id);
        // Sync URL to /artwork/[id] without triggering a route change.
        // This makes the lightbox shareable + browser Back closes it.
        window.history.pushState({ lightbox: true, id: work.id }, "", `/artwork/${work.id}`);
      }
    },
    [addRecent]
  );
  ...
  const closeLightbox = useCallback(() => {
    ...
    setCompareActive(false);
    document.body.style.overflow = "";
    // If we pushed a lightbox URL, go back so the URL reflects the gallery
  ...
  const lbNav = useCallback(
    (dir: number) => {
      setLightboxIndex((prev) => {
        const next = (prev + dir + lightboxItems.length) % lightboxItems.length;
        const work = lightboxItems[next];
        // Update URL to the new artwork (replaceState, not pushState, so Back
        // still closes the lightbox instead of stepping through every viewed artwork)
        if (work) {
          window.history.replaceState({ lightbox: true, id: work.id }, "", `/artwork/${work.id}`);
        }
        return next;
      });
      setIsZoomed(false);
